...
export default function CircularGallery({ items, bend = 3, borderRadius = 0.05, scrollSpeed = 2, scrollEase = 0.045 }: CircularGalleryProps) {
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ alpha: true, antialias: true, dpr: Math.min(window.devicePixelRatio || 1, 4) });
    ...
    const resize = () => {
      ...
      const planeHeight = (viewport.height * (1080 * scale)) / height;
  ...
  return <div ref={containerRef} className="circular-gallery" tabIndex={0} role="region" aria-label="科研成果画廊，可滚动或拖动浏览" />;
}
