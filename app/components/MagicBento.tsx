...
import { useRef, type CSSProperties, type MouseEvent, type PointerEvent, type RefObject, type WheelEvent } from "react";
...
export type MagicBentoItem = {
  ...
  descriptionStyle?: "bullets" | "paragraphs";
  href?: string;
  featured?: boolean;
  fit?: "cover" | "contain";
};
...
function MagicBentoCard({ item, glowColor, showCursor }: { item: MagicBentoItem; glowColor: string; showCursor: boolean }) {
  ...
  const onEnter = () => {
    ...
    if (cursorRef.current) gsap.to(cursorRef.current, { opacity: 1, scale: 1, duration: 0.18, ease: "power2.out" });
  };
  const onMove = (event: MouseEvent<HTMLElement>) => {
    ...
    if (cursorRef.current) gsap.to(cursorRef.current, { x: x - rect.width / 2, y: y - rect.height / 2, duration: 0.12, ease: "power2.out" });
  ...
  const onLeave = () => {
    clearParticles();
    if (cursorRef.current) gsap.to(cursorRef.current, { opacity: 0, scale: 0.75, duration: 0.18, ease: "power2.in" });
  ...
  const content = item.image ? <>{item.fit === "contain" && <span className="magic-bento-photo-backdrop" style={{ backgroundImage: `url("${item.image}")` }} />}<img src={item.image} alt={item.alt} loading="eager" decoding="sync" /><span className="magic-bento-shade" />{showCursor && <span ref={cursorRef} className="magic-bento-cursor">VIEW</span>}{(item.label || item.title) && <div className="magic-bento-copy">{item.label && <small>{item.label}</small>}{item.title && <h3>{item.title}</h3>}</div>}</> : <div className="magic-bento-work-copy"><span className="magic-bento-period">{item.period}</span><h3>{item.title}</h3><b>{item.label}</b>{Array.isArray(item.description) ? item.descriptionStyle === "paragraphs" ? <div className="magic-bento-paragraphs">{item.description.map((line) => <p key={line}>{line}</p>)}</div> : <ul>{item.description.map((line) => <li key={line}>{line}</li>)}</ul> : <p>{item.description}</p>}</div>;
...
export default function MagicBento({ items, className = "", scrollable = false, glowColor = "184, 223, 114", showCursor = false }: MagicBentoProps) {
  ...
  return <div ref={scrollerRef} className={`magic-bento-grid${scrollable ? " is-scrollable" : ""} ${className}`} onWheel={onWheel} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={finishDrag} onPointerCancel={finishDrag}>{items.map((item) => <MagicBentoCard key={item.image ?? item.title ?? item.alt} item={item} glowColor={glowColor} showCursor={showCursor} />)}</div>;
}
